...
#define REV_SIZE (64)
/* some extra versions in ubuntu xenial are 37 characters long
 * +deb1+cvs20081104-13.1ubuntu0.16.04.1
 * daily13.06.05+16.04.20160809-0ubuntu1 */

static void trim_ver(char *ver, regex_t *preg, char **outver, char *rev)
{
	regmatch_t pmatch;
	char *pos, *sep, *lastchar;
...
/*	if (regexec(preg, *outver, 1, &pmatch, 0) == REG_NOMATCH) {
	...
	/* Remove anything that is no longer a digit or '.' */
	pos = *outver;
	lastchar = pos;
	while (pos != NULL && pos[0] != '\0') {
		if (isdigit(pos[0]) == 0 && pos[0] != '.') break;
		lastchar = pos;
		++pos;
	}
	if (pos != NULL && pos[0] != '\0') {
		if (lastchar[0] == '.') pos = lastchar;
		if (rev != NULL) memcpy(rev, pos, strlen(pos) + 1);
		pos[0] = '\0';
	}	
}
...
int main(int argc, char *argv[])
{
	 ...
	 * Too specific, won't trim stuff like "1.1.43-1+b2", "0.8.1-19+deb9u1",
	 * "1.6.7+ds-2", "1.0.0+dfsg-2+b1", "7.2dfsg-3" and "0.4.1ubuntu1"
	 * It looks like rev doesn't even always start with a + or -
	 */
...
print:
		...
		if ((strstr(fields[FIELD_NAME], "-dbg") != NULL) &&
			(strstr(fields[FIELD_NAME], "libc6") == NULL)) goto cleanup;
			/* except libc6, needed for Valgrind */
		...
		if (strlen(fields[FIELD_SOURCE]) == 0)
			fputs(fields[FIELD_NAME], stdout);
		else
			fputs(fields[FIELD_SOURCE], stdout);
		putc('|', stdout);
		fputs(fields[FIELD_NAME], stdout);
		putc('|', stdout);
		fputs(ver, stdout);
